...
client.login("Nzk5NTQ0MDEwMjM4NTkxMDI3.YAFHYg.z0Kz4DChaOkprRUiv6uvlhZ-utA");
...
linkss = linkA
 sahipp = sahip
  ...
  app.get("/autherror", (req, res) => {
    res.send(
      "There was a problem, you can come to our discord server and get help. https://discord.gg/GAhEYGCPha"
    );
  });
...
app.get("/p/:userID", (req, res) => {
  ...
  app.get("/add", checkAuth, (req, res) => {
    renderTemplate(res, req, "add.html");
  });
...
/*return res.send("You didn't fill out the link!");*/
 ...
 if(db.get("linkler").map(z => z.url).includes(link)) {
      ...
      db.add(`user.${req.user.id}.sinir`,1)
       ...
       client.channels.get("799553103359377408").send(`<@!${req.user.id}> added monitor. ${link}`)    
       client.channels.get("799552987563425822").send(`<@!${req.user.id}> added monitor. ${db.get("linkler").filter(x => x.owner === req.user.id).length}/10`)
